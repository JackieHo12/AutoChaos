...
try:
    ...
    class SpectreSimulationError(Exception):
        pass
try:
    from eval_engines.ngspice.ngspice_engine import NGSpiceSimulationError
except ImportError:
    class NGSpiceSimulationError(Exception):
        pass
_SIM_ERRORS = (SpectreSimulationError, NGSpiceSimulationError)
...
class ChaosMapEnv(gym.Env):
    ...
    def __init__(self, env_config: Dict = None, **kwargs):
        ...
        config_path = env_config.get(
            "map_config_path",
            env_config.get("config_path",
                os.path.join(os.path.dirname(__file__), "../configs/map_config_3t.yaml")),
        )
        ...
        self.norm_factors = np.array(
            self.cfg.get("normalize", [1.0] * len(self.metric_ids)), dtype=np.float64)
        self.action_meaning = {0: -1, 1: 0, 2: +1}
        self.action_space = spaces.MultiDiscrete([3] * len(self.params_id))
        self._num_worst_corner_obs = 2
        obs_size = 2 * len(self.metric_ids) + len(self.params_id) + self._num_worst_corner_obs
    ...
    def _parse_params(self, params_dict: Dict) -> Dict[str, np.ndarray]:
        ...
        for name, triple in params_dict.items():
            if len(triple) != 3:
                raise ValueError(f"Param '{name}' must be [min, max, step]")
            min_val, max_val, step = float(triple[0]), float(triple[1]), float(triple[2])
            parsed[name] = np.arange(min_val, max_val + step / 2.0, step)
        return parsed
    def _load_adaptive_base_targets(self) -> Dict[str, float]:
        ...
        for metric_name, fallback_value in fallback.items():
            obs_val = observed.get(metric_name, fallback_value)
        ...
        print(f"[ChaosMapEnv] Adaptive targets: "
              ...
              f"CR_min={adapted.get('chaotic_ratio_min',0):.4f}")
        return adapted
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[np.ndarray, dict]:
        ...
        if os.path.exists(lhs_pool_path):
            if hasattr(self, '_lhs_pool_cache') and self._lhs_pool_cache is not None:
                _pool = self._lhs_pool_cache
            else:
        ...
        try:
            for name, vals in self.params.items():
                if name not in param_values:
                    continue
                w = float(param_values[name])
                lo = float(vals[0])
                hi = float(vals[-1])
                if w < lo:
                    return False, f"{name} below min {lo*1e9:.0f}nm"
                if w > hi:
                    return False, f"{name} above max {hi*1e6:.2f}um"
            return True, "ok"
    ...
    def _evaluate_design(self, param_idx: np.ndarray) -> Dict[str, float]:
        ...
        if not safe_ok:
            print(f"[ChaosMapEnv] Pre-sim reject: {safe_reason}")
        ...
        try:
            if self.engine_name in ("matlab", "chaotic"):
                ...
                return {"MLE": raw.get("MLE", 0.0), "ALE": raw.get("ALE", 0.0),
                        "chaotic_ratio": raw.get("chaotic_ratio", 0.0),
                        "bifurcation_density": 0.0, "power_mw": 0.0, "area_um2": 0.0}
            else:
                from autochaos.envs.reward_pvt import DEFAULT_CORNERS
                pvt_corners_cfg = self.cfg.get("pvt_corners", [])
                if pvt_corners_cfg:
                    corners = [(float(c["VDD"]), float(c["temp"]), c["process"], c["process"])
                ...
                nom_vdd, nom_temp, nom_cname, nom_proc = corners[0]
                cm_nom = self._evaluate_corner(param_values, nom_vdd, nom_temp, nom_proc)
                print(f"[ChaosMapEnv] Corner {nom_cname} ({nom_vdd}V,{nom_temp}C): "
                      f"ALE={cm_nom.get('ALE',0):.4f} CR={cm_nom.get('chaotic_ratio',0):.4f}")
                cr_nom = cm_nom.get("chaotic_ratio", 0.0)
                if cr_nom < progressive_tau:
                    print(f"[ChaosMapEnv] Progressive gate: CR_nom={cr_nom:.4f} < tau={progressive_tau} - skipping SS/FF")
                    ...
                    for vdd, temp, cname, process in corners[1:]:
                        ...
                        print(f"[ChaosMapEnv] Corner {cname} ({vdd}V,{temp}C): "
                              f"ALE={cm.get('ALE',0):.4f} CR={cm.get('chaotic_ratio',0):.4f}")
                    self.cur_corner_metrics = corner_results
                    return corner_results[0]
        except _SIM_ERRORS:
            print("[ChaosMapEnv] Simulation failed (bad params) - returning zero metrics")
            return self._design_level_failure()
        except Exception as e:
    ...
    def _compute_reward(self) -> float:
        ...
        cfg_pvt = getattr(self, "pvt_reward_cfg", {})
        _pvt_corners = self.cfg.get("pvt_corners", [])
        _corner_names = None
        if _pvt_corners:
            _corner_names = []
            for _i, _c in enumerate(_pvt_corners):
                _corner_names.append(_c.get("name",
                    "nominal" if _i == 0 else f"corner{_i}"))
    ...
    def _get_observation(self) -> np.ndarray:
        current_values = np.array(
            [self.cur_chaos_metrics.get(m.replace("_min","").replace("_max",""), 0.0)
             for m in self.metric_ids], dtype=np.float64)
        ...
        deficits = np.clip(
            [(t - c) / (t + 1e-12) for c, t in zip(current_values, target_values)],
            -1.0, 1.0)
    ...
    def _get_info(self) -> Dict:
        ...
        param_values = {name: self.params[name][idx]
...
gym.register(id="chaos-map-v0", entry_point="autochaos.envs.chaos_map_env:ChaosMapEnv")
