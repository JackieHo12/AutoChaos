...
class AutoChaosCallbacks(RLlibCallback):
    ...
    def on_episode_end(self, *, episode, metrics_logger=None, **kwargs):
        ...
        try:
            ...
            metrics_logger.log_value(('chaos', 'cr_nominal'),    cr_nom,               reduce='max',  window=100)
            metrics_logger.log_value(('chaos', 'cr_nominal_iter'), cr_nom, reduce='max', window=20)
            metrics_logger.log_value(('chaos', 'ale_nominal'),   ale_nom,              reduce='max',  window=100)
            metrics_logger.log_value(('chaos', 'cr_worst'),      min_cr,               reduce='max',  window=100)
            metrics_logger.log_value(('chaos', 'cr_worst_iter'), min_cr, reduce='max', window=20)
            metrics_logger.log_value(('chaos', 'ale_worst'),     min_ale,              reduce='max',  window=100)
            metrics_logger.log_value(('chaos', 'all_pass_rate'), 1.0 if all_pass else 0.0,
                                     reduce='mean', window=100)
            if area_um2 > 0.0:
                metrics_logger.log_value(('chaos', 'area_um2'), area_um2, reduce='mean', window=100)
        except Exception as e:
            print('[AutoChaosCallbacks] on_episode_end warning: ' + str(e))
    def on_env_runners_recreated(self, *, algorithm, env_runner_group,
                                  env_runner_indices, is_evaluation, **kwargs):
        n = len(env_runner_indices)
        label = 'eval' if is_evaluation else 'train'
        print('[AutoChaosCallbacks] ' + str(n) + ' ' + label +
              ' EnvRunner(s) recreated: indices=' + str(env_runner_indices))
        try:
            if hasattr(algorithm, 'metrics') and algorithm.metrics is not None:
                algorithm.metrics.log_value(('fault', 'workers_recreated'), float(n), reduce='sum')
        except Exception as e:
    ...
    def on_train_result(self, *, algorithm, metrics_logger=None, result, **kwargs):
        try:
            ...
            def _set_tau(worker):
                try:
                    env = worker.env.envs[0] if hasattr(worker.env, 'envs') else worker.env
                    if hasattr(env, 'cfg'):
