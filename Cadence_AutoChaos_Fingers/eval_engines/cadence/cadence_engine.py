...
class CadenceEngine:
    ...
    def __init__(self, config: Dict):
        ...
        self._finger_mode = bool(config.get('finger_mode', map_cfg.get('finger_mode', False)))
        self._finger_step = float(config.get('finger_step', map_cfg.get('finger_step', 120e-9)))
        if self._finger_mode:
            print(f'[CadenceEngine]   FINGER MODE ON, finger_step={self._finger_step}')
    ...
    def _apply_fingers(self, content: str, params: Dict) -> str:
        step = float(self._finger_step)
        def _w_of(token):
            ref = re.match(r'(W_\w+)', token)
            if ref and ref.group(1) in params:
                return float(params[ref.group(1)])
            lm = re.match(r'\(?([\d.]+)([a-z]?)\)?', token)
            if lm:
                mult = {'':1,'n':1e-9,'u':1e-6,'p':1e-12,'m':1e-3}.get(lm.group(2),1)
                return float(lm.group(1))*mult
            return None
        out = []
        for line in content.splitlines():
            if re.search(r'g45[np]1lvt', line):
                m = re.search(r'\sw=(\S+)', line)
                if m:
                    w = _w_of(m.group(1))
                    if w and w > 0:
                        nf = max(1, round(w*1e12) // round(step*1e12))
                        line = re.sub(r'\snf=\S+', f' nf={nf}', line)
            out.append(line)
        return "\n".join(out)
    def _create_netlist(self, params: Dict, netlist_path: str):
        ...
        if getattr(self, '_finger_mode', False):
            content = self._apply_fingers(content, params)
