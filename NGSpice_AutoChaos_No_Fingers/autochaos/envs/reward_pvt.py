...
def compute_pvt_reward(corner_metrics, tau_ale=DEFAULT_TAU_ALE, tau_cr=DEFAULT_TAU_CR,
                       ...
                       nom_bonus=DEFAULT_NOM_BONUS, corner_names=None):
    ...
    for i, cm in enumerate(corner_metrics):
        ...
        if _sim_failed:
            dbg_c = dict(dbg_c)
            dbg_c["SIM_FAILED"] = True
        corner_debug.append({cname: dbg_c})
        if _sim_failed or ale < tau_ale or cr < tau_cr:
            all_pass = False
        if (not _sim_failed) and (ale > 0.0 or cr > 0.0):
            all_dead = False
    if not valid_ales:
    ...
    r_nom_bonus = nom_bonus if (n == 1 and (crs[0] if crs else 0.0) >= 1.0
                                and (ales[0] if ales else 0.0) >= tau_ale) else 0.0
