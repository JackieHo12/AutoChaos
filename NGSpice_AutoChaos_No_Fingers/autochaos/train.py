...
CHECKPOINT_DIR = os.path.join(os.path.abspath("runs"), "checkpoints")
CHECKPOINT_FREQ = 10
...
def run_validate(env_config, episodes):
    ...
    for ep in range(episodes):
        ...
        print(f"Episode {ep+1}/{episodes} reward={total_reward:.3f} steps={steps} elapsed={time.perf_counter()-ep_start:.1f}s")
    env.close()
    print(f"[train.py] TOTAL VALIDATE ELAPSED = {time.perf_counter()-total_start:.1f}s")
...
def run_training(env_config, train_cfg, args):
    ...
    for i in range(args.episodes):
        ...
        algo.env_runner_group.foreach_env_runner(
            lambda w: setattr(
                w.env.envs[0] if hasattr(w.env, "envs") else w.env,
                "training_iter", i
            ) if hasattr(w, "env") and w.env is not None else None
        )
        result = algo.train()
    ...
    print(f"[train.py] TOTAL TRAIN ELAPSED = {time.perf_counter()-total_start:.1f}s")
